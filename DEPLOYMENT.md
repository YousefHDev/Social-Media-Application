...
- **Current Setup**: Socket.IO is not included in the codebase.
- **Limitation**: Vercel Serverless Functions execute state-free and time-limited HTTP invocations; they do not maintain long-lived TCP/WebSocket connections.
- **Production Recommendation**: If real-time features (chat, live notifications) are required in the future, use a serverless WebSocket provider like **Pusher**, **Ably**, or **Supabase Realtime**, or host a dedicated WebSocket service on **Render** or **Railway**.
